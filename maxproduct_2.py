...
class TreeMaxSumInference:
    ...
    def nodeMessage(self,node_id):            
        ...
        for node_id2 in self.fg.neighbours(node_id):
            ...
            # envoie de message
            if debug_2 :
              print("Node message  : {} du noeud {} au factor {}".format(message,node_id,node_id2))
    ...
    def factorMessage(self,node_id):
        ...
        for node_id2 in self.fg.neighbours(node_id):
            ...
            if self.dict_dict_cpt[node_id2][node_id] != None:
                continue;
            # init message
            message = self.logCpt(self.fg.node[node_id]);
            # pour tous les voisins sauf node_id2
            flag = True
            for node_id3 in self.fg.neighbours(node_id):
                # si c'est node_id2
                if node_id2 == node_id3:
                    continue;
                # si node_id ne peut pas envoyer de message a node2, i.e. il mqnaue un message d'un noeud node_id3
                if self.dict_dict_cpt[node_id][node_id3] == 0:
                    continue;
                else:
                    if self.dict_dict_cpt[node_id][node_id3] == None:
                        flag = False
            ...
            # envoie de message
            if debug_2 :
              print("Factor message  : {} du factor {} au noeud {}".format(message,node_id,node_id2))
            self.dict_dict_cpt[node_id2][node_id] = message;
    ...
    def posterior(self,node_id):
        ...
        cpt.normalize()
        if debug_3 :
            print("la probabilité jointe du node {} est {} ".format(node_id,cpt))       
        ...
        if debug_3 :        
            print("La probabilité marginale du node {} est {}".format(node_id,cpt))        
        return cpt.argmax()[0]
    ...
    def logCpt(self,cpt) :
        cptTmp = gum.Potential(cpt)
        for i in cptTmp.loopIn():
            a = cptTmp.get(i)
            cptTmp.set(i,np.log(a))
